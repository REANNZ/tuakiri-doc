...
To automatically release attributes to new services registered in the federation, we will configure the IdP to load additional attribute filter files releasing attributes based on information contained in the Service Provider metadata.  These filters were originally designed for use with eduGAIN, but also handle attribute release within Tuakiri.
